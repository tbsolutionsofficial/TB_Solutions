...
export async function POST(req: NextRequest) {
  try {
    ...
    // Try email notification — non-fatal if credentials not configured
    const senderEmail = process.env.CONTACT_EMAIL_USER;
    const senderPass = process.env.CONTACT_EMAIL_PASSWORD;
    const receiverEmail = process.env.CONTACT_RECEIVER_EMAIL || "tbsolutions.official@gmail.com";

    const hasValidCreds =
      senderEmail &&
      senderPass &&
      senderPass !== "placeholder" &&
            ...
            <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;">
              ...
              <div style="background:#faf9f5;padding:24px;border-radius:0 0 12px 12px;border:1px solid #e6dfd8;">
                <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
                  <tr><td style="padding:8px 0;color:#6c6a64;font-size:13px;width:130px;">Name</td><td style="padding:8px 0;font-weight:600;">${name}</td></tr>
                  <tr><td style="padding:8px 0;color:#6c6a64;font-size:13px;">College/Company</td><td style="padding:8px 0;">${company || "—"}</td></tr>
                  ...
                  <tr><td style="padding:8px 0;color:#6c6a64;font-size:13px;">Phone</td><td style="padding:8px 0;">${phone}</td></tr>
                  <tr><td style="padding:8px 0;color:#6c6a64;font-size:13px;">Domain</td><td style="padding:8px 0;">${domain}</td></tr>
                </table>
                <hr style="border:none;border-top:1px solid #e6dfd8;margin:0 0 16px;" />
                <p style="color:#3d3d3a;line-height:1.7;font-size:14px;">${message.replace(/\n/g, "<br>")}</p>
              </div>
            </div>
          `,
        });
    ...
    // Always return success — contact is saved to Firestore from the client
